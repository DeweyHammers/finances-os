...
- Amber ⚠ (`WarningAmberIcon`, `#fbbf24`) appears next to a Bill envelope's name in `BudgetItemRow` when funded ≠ expected-through-current-pay-week (fires OVER **and** UNDER; zero tolerance since funding is integer cents throughout).
...
- **Per-occurrence overpay cap** — `activityInBillGraceWindowCents` clamps each grace window's raw activity at `bill.amount` before summing, so an overpayment on one firing (Sep 5 charged $101.66 against a $101 estimate) doesn't silently reduce the "still-to-save" target for another firing in the same view (Oct 5 forward-extension). Sync targets the Overview plan verbatim; real overspends are a checking-account matter, not envelope-level bookkeeping. Only overpayments are capped — underpayments pass through so an envelope that ended up under-spent doesn't flag as needing more funding.
- **Payday suppression**: when the PREVIOUS pay week had a NON-ZERO target that was already met (`expectedPreviousAdjusted > 0 && fundedCents >= expectedPreviousAdjusted - tolerance`), under-fund shortfalls against the current period are silenced — treated as "the new pay week just started and hasn't been assigned yet" rather than real drift. The non-zero guard is critical: a single-week bill (Gemini due 20th, Discord due 17th) has its entire slice in the current pay week, so previous target = 0. Without the guard, `funded >= 0` would trivially satisfy previousMet and silence a genuinely-unfunded envelope. Overview-change drift naturally breaks previousMet (splits redistribute) so warnings return on real drift. Over-fund always fires (no payday-timing ambiguity).
